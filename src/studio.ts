...
function revealFallback(): void {
  document.querySelectorAll<HTMLElement>('.kicker,.title,.desc,.actions,.micro,.label,.section-title,.section-body,.principle,.list li,.contact').forEach((el) => {
    el.style.opacity = '1';
...
function initPreloader(): void {
  ...
  const tick = (now: number): void => {
    ...
    if (status) status.textContent = progress < .24 ? 'CALIBRATING' : progress < .5 ? 'CONNECTING' : progress < .78 ? 'RENDERING' : 'READY';
...
function initScroll(): void {
  ...
  try {
    ...
    console.warn('Locomotive Scroll unavailable; native scroll retained.', error);
    initScrollScenes();
  }
}
...
function initOrbit(): void {
  ...
  nodes.forEach((node, index) => {
    const state = { angle: index * 1.25, radius: [.34,.37,.32,.36,.29][index] ?? .34 };
    gsap.to(state, { angle: state.angle + Math.PI * 2, duration: 34 + index * 9, repeat: -1, ease: 'none', onUpdate: () => { node.style.left = `${50 + Math.cos(state.angle) * state.radius * 100}%`; node.style.top = `${50 + Math.sin(state.angle) * state.radius * 72}%`; } });
  });
  if (pulse) {
    const pulseLoop = (): void => { gsap.timeline().to(pulse, { scale: 2.6, opacity: .3, duration: .9, ease: 'power2.out' }).to(pulse, { scale: 4.5, opacity: 0, duration: 1.5, ease: 'power2.in' }); gsap.delayedCall(12, pulseLoop); };
    gsap.delayedCall(5, pulseLoop);
  }
}
...
function initCursor(): void {
  ...
  document.querySelectorAll<HTMLElement>('a,.principle,.list li').forEach((el) => {
  ...
  const loop = (): void => { rx += (x - rx) * .14; ry += (y - ry) * .14; dot.style.transform = `translate3d(${x}px,${y}px,0)`; ring.style.transform = `translate3d(${rx}px,${ry}px,0)`; requestAnimationFrame(loop); };
  loop();
}
